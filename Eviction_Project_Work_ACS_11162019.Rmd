...
```{r set up state spatial datasets}
...
# create table with all field averages
...
tm_shape(state_all_avgs) +
  tm_polygons("avg_filing_rate") 

tm_shape(county_all_avgs) +
  tm_polygons("avg_filing_rate")

```
```{r create data breaks - state map}
pretty_breaks <- c(5, 10, 15, 20, 25, 30)
# find the extremes
...
```{r attempt to use ggplot for maps}

ggplot(data = state_all_avgs) +
  geom_sf(aes(fill = brks), color = "grey", lwd = 0.01) +
  coord_sf(crs = st_crs(102003)) +
  theme(legend.position = "bottom") +
  scale_fill_manual(values = rev(magma(6)), 
...
```{r  ggplot for counties}

unique(county_all_avgs$avg_filing_rate)

pretty_breaks <- c(10, 20, 30 , 40, 50, 110)
# find the extremes
...
ggplot(data = county_all_avgs) +
  geom_sf(aes(fill = brks), color = "grey", lwd = 0) +
  coord_sf(crs = st_crs(102003)) +
  theme(legend.position = "bottom") +
  scale_fill_manual(values = rev(magma(6)), 
                  breaks = rev(brks_scale), 
...
write.csv(eviction_state, ".//state_map_data")
...
select_layers <- state_map_data %>%
                    select(poverty_rate, pct_renter_occupied, eviction_filing_rate, rent_burden)

...
```{r K-means creating cluster, echo=TRUE}
...
dplyr::select(eviction_county, region,eviction_filing_rate, year) %>%
  filter(year>2009) %>%
...
```{r pct change metric}
#pct_change metric
dplyr::select(eviction_county, year, GEOID,population,eviction_filing_rate) %>%
...
left_join(pct_pop_change_county,eviction_county, by ="GEOID") %>%
  filter(year.x > 2004) %>%
  dplyr::select(year.x,pct_pop_change,eviction_filing_rate.x,region) %>%

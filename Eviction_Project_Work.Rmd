...
```{r MP Distribution of Variables}
eviction_county %>%
  select(-year,-`eviction-filing-rate`,-`eviction-filings`, -evictions,-imputed, - `low-flag`,-subbed) %>% 
...
```{r MP plotting Y against X}
na.omit(eviction_county) %>%
  select_if(is.numeric) %>%
  gather(-`eviction-rate`,-population,-`rent-burden`, key = "var", value = "value") %>%
  ggplot(aes(x = value, y = `eviction-rate`, color = population)) +
    geom_point() +
...
```{r AR remove_dash, eval=FALSE}
# remove '-' from column names
# for (i in names(eviction_county)) {
#   if (grepl("-", i, fixed=TRUE)) {
#     names(eviction_county) <- gsub("-", "_", i)
#   }
# }
# 

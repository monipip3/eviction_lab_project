...
# pseudo code for app
km <- eviction_county %>%
  dplyr::select(c(GEOID, population)) %>%
  filter(year == "2016") %>%
  distinct() %>%
  drop_na() %>%
  column_to_rownames(., var = "GEOID")
ind <- sapply(km, is.numeric)
km[ind] <- lapply(km[ind], scale)
set.seed(123)
km3 <- kmeans(km, 3, nstart = 25)
...
merge(km_avg, evr, by = "GEOID") %>%
  ggplot() + 
  geom_bar(aes(x = GEOID, y = avg_fl), stat = 'identity') + 
  facet_grid(. ~ cluster) 
```

```{r kmeans_avg_cluster_eda}

...
# Sort in descending order and grab the GEOIDs with the lowest filing rates -- ADJUST Y SCALE
bot_ec <- ecplot %>% 
  group_by(GEOID) %>% 
  arrange(avg_ev_fl_rate)
bot_ec <- c(unique(bot_ec$GEOID))[1:10]
# Plot the top and bottom filing counties over time
ecplot %>% 
  ...
  labs(title = "Highest Filing Counties Over Time",
...
ecplot %>% 
  ...
  theme_bw() + 
  ylim(0, 1)
# Plot the top and bottom filing counties over time colored by rural flag 
ecplot %>% 
  filter(GEOID %in% top_ec) %>% 
  ggplot() +
  geom_line(aes(x = year, 
...
```{r trends_region}
# Calculate top and bottom for each region
top_south <- ecplot %>%
  filter(region == "South") %>%
  group_by(GEOID) %>% 
  arrange(desc(avg_ev_fl_rate))
...
bot_south <- ecplot %>%
  ...
  group_by(GEOID) %>% 
  arrange(avg_ev_fl_rate)
bot_south <- c(unique(bot_south$GEOID))[1:10]
top_northeast <- ecplot %>%
  filter(region == "Northeast") %>%
  group_by(GEOID) %>% 
  arrange(desc(avg_ev_fl_rate))
top_northeast <- c(unique(top_northeast$GEOID))[1:10]
bot_northeast <- ecplot %>%
  filter(region == "Northeast") %>%
  group_by(GEOID) %>% 
  arrange(avg_ev_fl_rate)
bot_northeast <- c(unique(bot_northeast$GEOID))[1:10]
top_north_central <- ecplot %>%
  filter(region == "North Central") %>%
  group_by(GEOID) %>% 
  arrange(desc(avg_ev_fl_rate))
top_north_central <- c(unique(top_north_central$GEOID))[1:10]
bot_north_central <- ecplot %>%
  filter(region == "North Central") %>%
  group_by(GEOID) %>% 
  arrange(avg_ev_fl_rate)
bot_north_central <- c(unique(bot_north_central$GEOID))[1:10]
top_west <- ecplot %>%
  filter(region == "West") %>%
  group_by(GEOID) %>% 
  arrange(desc(avg_ev_fl_rate))
...
bot_west <- ecplot %>%
  ...
  group_by(GEOID) %>% 
  arrange(avg_ev_fl_rate)
bot_west <- c(unique(bot_west$GEOID))[1:10]
# Plot regional tops and bottoms
tplot_south <- ecplot %>% 
  ...
  labs(title = "Highest Filing South Counties",
       caption = "Data from evictionlab.org") + 
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_south <- ecplot %>% 
  filter(GEOID %in% bot_south) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  labs(title = "Lowest Filing South Counties",
       caption = "Data from evictionlab.org") + 
  scale_color_brewer(palette = "Spectral") +
  theme_bw() + 
  ylim(0, 1)
tplot_northeast <- ecplot %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  labs(title = "Highest Filing Northeast Counties",
       caption = "Data from evictionlab.org") + 
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_northeast <- ecplot %>% 
  filter(GEOID %in% bot_northeast) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  labs(title = "Lowest Filing Northeast Counties",
       caption = "Data from evictionlab.org") + 
  scale_color_brewer(palette = "Spectral") +
  theme_bw() +
  ylim(0, 1)
tplot_north_central <- ecplot %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  labs(title = "Highest Filing North Central Counties",
       caption = "Data from evictionlab.org") + 
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_north_central <- ecplot %>% 
  filter(GEOID %in% bot_north_central) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  labs(title = "Lowest Filing North Central Counties",
       caption = "Data from evictionlab.org") + 
  scale_color_brewer(palette = "Spectral") +
  theme_bw() + 
  ylim(0, 1)
tplot_west <- ecplot %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  labs(title = "Highest Filing West Counties",
       caption = "Data from evictionlab.org") + 
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_west <- ecplot %>% 
  filter(GEOID %in% bot_west) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  labs(title = "Lowest Filing West Counties",
       caption = "Data from evictionlab.org") + 
  scale_color_brewer(palette = "Spectral") +
  theme_bw() +
  ylim(0, 1)
# display top/bottom for each region in one plot 
grid.arrange(tplot_south, tplot_northeast, tplot_north_central, tplot_west)
grid.arrange(bplot_south, bplot_northeast, bplot_north_central, bplot_west)
```

```{r demos_all}
# Demographics for top and bottom (without pop. restriction)
# poverty distribution 
ecplot %>%
  filter(GEOID %in% top_south | GEOID %in% top_north_central | GEOID %in% top_northeast | GEOID %in% top_west) %>%
  group_by(GEOID) %>%
  ...
  geom_density(alpha=.5, fill=color_pal[[1]]) + 
  theme_bw() + 
  xlab("Poverty Rate") + 
  ylab("Density") + 
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Poverty Rates in Highest Filing Counties")
ecplot %>%
  filter(GEOID %in% bot_south | GEOID %in% bot_northeast | GEOID %in% bot_north_central | GEOID %in% bot_west) %>%
  group_by(GEOID) %>%
  mutate(avg_pov = mean(poverty_rate, na.rm=TRUE)) %>%
  ggplot(aes(x=avg_pov)) + 
  geom_histogram(aes(y=..density..), color="black", fill="white", bins=20) + 
  ...
  ylab("Density") + 
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Poverty Rates in Lowest Filing Counties")
...
ecplot %>%
  ...
  ggplot(aes(x=avg_rb)) + 
  geom_histogram(aes(y=..density..), color="black", fill="white", bins=20) + 
  geom_density(alpha=.5, fill=color_pal[[2]]) + 
  theme_bw() + 
  xlab("Rent Burden") + 
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Rent Burden in Highest Filing Counties")
ecplot %>%
  filter(GEOID %in% bot_south | GEOID %in% bot_northeast | GEOID %in% bot_north_central | GEOID %in% bot_west) %>%
  group_by(GEOID) %>%
  mutate(avg_rb = mean(rent_burden, na.rm=TRUE)) %>%
  ...
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Rent Burden in Lowest Filing Counties")
...
ecplot %>%
  ...
  geom_histogram(aes(y=..density..), color="black", fill="white", bins=20) + 
  geom_density(alpha=.5, fill=color_pal[[3]]) + 
  theme_bw() + 
  xlab("% White") + 
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "% White in Highest Filing Counties") 
ecplot %>%
  filter(GEOID %in% bot_south | GEOID %in% bot_northeast | GEOID %in% bot_north_central | GEOID %in% bot_west) %>%
  group_by(GEOID) %>%
  mutate(avg_pct_white = mean(pct_white, na.rm=TRUE)) %>%
  ...
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "% White in Lowest Filing Counties") 
...
ecplot %>%
  ...
  geom_histogram(aes(y=..density..), color="black", fill="white", bins=20) + 
  geom_density(alpha=.5, fill=color_pal[[4]]) + 
  theme_bw() + 
  xlab("% Nonwhite") + 
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "% Nonwhite in Highest Filing Counties") 
ecplot %>%
  filter(GEOID %in% bot_south | GEOID %in% bot_northeast | GEOID %in% bot_north_central | GEOID %in% bot_west) %>%
  group_by(GEOID) %>%
  mutate(avg_pct_nonwhite = mean(pct_nonwhite, na.rm=TRUE)) %>%
  ...
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "% Nonwhite in Lowest Filing Counties") 
...
ecplot %>%
  ...
  geom_histogram(aes(y=..density..), color="black", fill="white", bins=20) + 
  geom_density(alpha=.5, fill=color_pal[[5]]) + 
  theme_bw() + 
  xlab("Median Household Income") + 
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Median Household Income in Highest Filing Counties") 
ecplot %>%
  filter(GEOID %in% bot_south | GEOID %in% bot_northeast | GEOID %in% bot_north_central | GEOID %in% bot_west) %>%
  group_by(GEOID) %>%
  mutate(avg_med_hh = mean(median_household_income, na.rm=TRUE)) %>%
  ...
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Median Household Income in Lowest Filing Counties") 
...
ecplot %>%
  ...
  geom_histogram(aes(y=..density..), color="black", fill="white", bins=20) + 
  geom_density(alpha=.5, fill=color_pal[[6]]) + 
  theme_bw() +  
  xlab("Unemployment Rate") + 
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Unemployment Rate in Highest Filing Counties") 
ecplot %>%
  filter(GEOID %in% bot_south | GEOID %in% bot_northeast | GEOID %in% bot_north_central | GEOID %in% bot_west) %>%
  group_by(GEOID) %>%
  mutate(avg_ue_rt = mean(unemployment_rate, na.rm=TRUE)) %>%
  ...
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Unemployment Rate in Lowest Filing Counties") 
...
ecplot %>%
  ...
  geom_histogram(aes(y=..density..), color="black", fill="white", bins=20) + 
  geom_density(alpha=.5, fill=color_pal[[7]]) + 
  theme_bw() +  
  xlab("% Renter Occupied") + 
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "% Renter Occupied in Highest Filing Counties") 
ecplot %>%
  filter(GEOID %in% bot_south | GEOID %in% bot_northeast | GEOID %in% bot_north_central | GEOID %in% bot_west) %>%
  group_by(GEOID) %>%
  mutate(avg_pct_ro = mean(pct_renter_occupied, na.rm=TRUE)) %>%
  ...
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "% Renter Occupied in Lowest Filing Counties") 
# pct_rural dist
ecplot %>%
  filter(GEOID %in% top_south | GEOID %in% top_north_central | GEOID %in% top_northeast | GEOID %in% top_west) %>%
  group_by(GEOID) %>%
  ...
  geom_histogram(aes(y=..density..), color="black", fill="white", bins=20) + 
  geom_density(alpha=.5, fill=color_pal[[8]]) + 
  theme_bw() +  
  xlab("% Renter Occupied") + 
  ylab("Density") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
# Sort in descending order and grab the GEOIDs with the highest filing rates
top_ec_pop <- ecplot_gte_mean %>% 
  group_by(GEOID) %>% 
  arrange(desc(avg_ev_fl_rate))
top_ec_pop <- c(unique(top_ec_pop$GEOID))[1:10]
# Sort in descending order and grab the GEOIDs with the lowest filing rates 
bot_ec_pop <- ecplot_gte_mean %>% 
  group_by(GEOID) %>% 
  arrange(avg_ev_fl_rate)
bot_ec_pop <- c(unique(bot_ec_pop$GEOID))[1:10]
# Plot the top and bottom filing counties over time
ecplot_gte_mean %>% 
  filter(GEOID %in% top_ec_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  labs(title = "Highest Filing Counties* Over Time",
       subtitle = "*among populations greater than the mean",
       caption = "Data from evictionlab.org") + 
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
ecplot_gte_mean %>% 
  filter(GEOID %in% bot_ec_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
...
top_south_pop <- ecplot_gte_mean %>%
  filter(region == "South") %>%
  group_by(GEOID) %>% 
  arrange(desc(avg_ev_fl_rate))
top_south_pop <- c(unique(top_south_pop$GEOID))[1:10]
bot_south_pop <- ecplot_gte_mean %>%
  filter(region == "South") %>%
  group_by(GEOID) %>% 
  arrange(avg_ev_fl_rate)
bot_south_pop <- c(unique(bot_south_pop$GEOID))[1:10]
top_northeast_pop <- ecplot_gte_mean %>%
  filter(region == "Northeast") %>%
  group_by(GEOID) %>% 
  arrange(desc(avg_ev_fl_rate))
top_northeast_pop <- c(unique(top_northeast_pop$GEOID))[1:10]
bot_northeast_pop <- ecplot_gte_mean %>%
  filter(region == "Northeast") %>%
  group_by(GEOID) %>% 
  arrange(avg_ev_fl_rate)
bot_northeast_pop <- c(unique(bot_northeast_pop$GEOID))[1:10]
top_north_central_pop <- ecplot_gte_mean %>%
  filter(region == "North Central") %>%
  group_by(GEOID) %>% 
  arrange(desc(avg_ev_fl_rate))
top_north_central_pop <- c(unique(top_north_central_pop$GEOID))[1:10]
bot_north_central_pop <- ecplot_gte_mean %>%
  filter(region == "North Central") %>%
  group_by(GEOID) %>% 
  arrange(avg_ev_fl_rate)
bot_north_central_pop <- c(unique(bot_north_central_pop$GEOID))[1:10]
top_west_pop <- ecplot_gte_mean %>%
  filter(region == "West") %>%
  group_by(GEOID) %>% 
  arrange(desc(avg_ev_fl_rate))
top_west_pop <- c(unique(top_west_pop$GEOID))[1:10]
bot_west_pop <- ecplot_gte_mean %>%
  filter(region == "West") %>%
  group_by(GEOID) %>% 
  arrange(avg_ev_fl_rate)
bot_west_pop <- c(unique(bot_west_pop$GEOID))[1:10]
# Plot regional tops and bottoms
tplot_south_pop <- ecplot_gte_mean %>% 
  filter(GEOID %in% top_south_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_south_pop <- ecplot_gte_mean %>% 
  filter(GEOID %in% bot_south_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  theme_bw() + 
  ylim(0, 1)
tplot_northeast_pop <- ecplot_gte_mean %>% 
  filter(GEOID %in% top_northeast_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_northeast_pop <- ecplot_gte_mean %>% 
  filter(GEOID %in% bot_northeast_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  theme_bw() +
  ylim(0, 1)
tplot_north_central_pop <- ecplot_gte_mean %>% 
  filter(GEOID %in% top_north_central_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_north_central_pop <- ecplot_gte_mean %>% 
  filter(GEOID %in% bot_north_central_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  theme_bw() + 
  ylim(0, 1)
tplot_west_pop <- ecplot_gte_mean %>% 
  filter(GEOID %in% top_west_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_west_pop <- ecplot_gte_mean %>% 
  filter(GEOID %in% bot_west_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
                group = GEOID, 
  ...
  ylim(0, 1)
# display top/bottom for each region in one plot 
grid.arrange(tplot_south_pop, tplot_northeast_pop, tplot_north_central_pop, tplot_west_pop)
grid.arrange(bplot_south_pop, bplot_northeast_pop, bplot_north_central_pop, bplot_west_pop)
```

```{r trends_regional_gte_pop_rural}
# Plot regional tops and bottoms
tplot_south_pop_rural <- ecplot_gte_mean %>% 
  filter(GEOID %in% top_south_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
  ...
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_south_pop_rural <- ecplot_gte_mean %>% 
  filter(GEOID %in% bot_south_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
  ...
  theme_bw() + 
  ylim(0, 1)
tplot_northeast_pop_rural <- ecplot_gte_mean %>% 
  filter(GEOID %in% top_northeast_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
  ...
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_northeast_pop_rural <- ecplot_gte_mean %>% 
  filter(GEOID %in% bot_northeast_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
  ...
  theme_bw() +
  ylim(0, 1)
tplot_north_central_pop_rural <- ecplot_gte_mean %>% 
  filter(GEOID %in% top_north_central_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
  ...
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_north_central_pop_rural <- ecplot_gte_mean %>% 
  filter(GEOID %in% bot_north_central_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
  ...
  theme_bw() + 
  ylim(0, 1)
tplot_west_pop_rural <- ecplot_gte_mean %>% 
  filter(GEOID %in% top_west_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
  ...
  scale_color_brewer(palette = "Spectral") +
  theme_bw()
bplot_west_pop_rural <- ecplot_gte_mean %>% 
  filter(GEOID %in% bot_west_pop) %>% 
  ggplot() +
  geom_line(aes(x = year, 
                y = eviction_filing_rate, 
  ...
  ylim(0, 1)
# display top/bottom for each region in one plot 
grid.arrange(tplot_south_pop_rural, tplot_northeast_pop_rural, tplot_north_central_pop_rural, tplot_west_pop_rural)
grid.arrange(bplot_south_pop_rural, bplot_northeast_pop_rural, bplot_north_central_pop_rural, bplot_west_pop_rural)
```
...
# poverty distribution 
ecplot_gte_mean %>%
  filter(GEOID %in% top_south_pop | GEOID %in% top_north_central_pop | GEOID %in% top_northeast_pop | GEOID %in% top_west_pop) %>%
  group_by(GEOID) %>%
  mutate(avg_pov = mean(poverty_rate, na.rm=TRUE)) %>%
  ggplot(aes(x=avg_pov)) + 
  geom_histogram(aes(y=..density..), color="black", fill="white", bins=20) + 
  ...
  labs(title = "Poverty Rates in Highest Filing Counties")
ecplot_gte_mean %>%
  filter(GEOID %in% bot_south_pop | GEOID %in% bot_north_central_pop | GEOID %in% bot_northeast_pop | GEOID %in% bot_west_pop) %>%
  group_by(GEOID) %>%
  mutate(avg_pov = mean(poverty_rate, na.rm=TRUE)) %>%
  ggplot(aes(x=avg_pov)) + 
  geom_histogram(aes(y=..density..), color="black", fill="white", bins=20) + 

...
```{r AR top_bot_cty_fl_trends, eval=FALSE}
...
### knn/kmeans expl. for determining similar counties
...
```{r knn_sim_cts}
# pseudo code
# use avg for ev_fl_rt 
# make df with just avg (1 row == 1 county?)
# NORMALIZE -- gotta put everything on the same scale -- **before** splitting
# all predictors should (and will be) numeric 
knn_ec <- eviction_county %>%
  group_by(GEOID) %>%
  mutate(avg_ev_fl_rate = mean(eviction_filing_rate, na.rm = TRUE)) %>%
  mutate(avg_pop = mean(population))

# quick test -- only use two params to cluster GEOID
knn_ec_test <- subset(knn_ec, select = c("GEOID", "avg_ev_fl_rate", "avg_pop"))

# dedupe and drop nulls
knn_ec_test <- knn_ec_test %>%
  distinct() %>%
  drop_na()

# normalize values
fl_sc <- scale(knn_ec_test$avg_ev_fl_rate)
pop_sc <- scale(knn_ec_test$avg_pop)
knn_ec_test$avg_ev_fl_rate <- fl_sc
knn_ec_test$avg_pop <- pop_sc

# train-test split
require(caTools)
set.seed(123)
sample <- sample.split(knn_ec_test,SplitRatio = 0.70)
knn_train <- subset(knn_ec_test, sample == TRUE)
knn_test <- subset(knn_ec_test, sample == FALSE)
train_target <- knn_train$GEOID
test_target <- knn_test$GEOID
library(class)
# run knn function
pr <- knn(knn_train,knn_test,cl=train_target,k=13)
...
# quick test -- only use two params to cluster GEOID
km_ec_test <- subset(km_ec, select = c("GEOID", "avg_ev_fl_rate", "avg_pop"))

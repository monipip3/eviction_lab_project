...
```{r write csvs}
setwd("~/Documents/GitHub/eviction_lab_project/Eviction_Shiny/Eviction_Shiny")

write_csv(eviction_state_2010,path = "./eviction_state_2010.csv")
...
eviction_by_county <- eviction_county_2010 %>%
...
```{r}
eviction_state_2010 <- read_csv("./eviction_state_2010.csv")

eviction_state_2010

spatial_state <- as_Spatial(state_us_geo)

df <- as.data.frame(spatial_state)
df
state_eviction <- merge(spatial_state, eviction_state_2010, by = c("GEOID" = "GEOID"), duplicateGeoms = TRUE)
                        
state_eviction

df2 <- as.data.frame(state_eviction)
df2 %>%
  filter(GEOID == '51' | GEOID == "39") %>%
  filter(year == 2010)

state_eviction2 <- sp.na.omit(state_eviction, margin = 1)
state_eviction2[state_eviction2$year == "2010", ]

test <- subset(state_eviction2, state_eviction2$year == 2010)
as.data.frame(test)

state_eviction2df <- as.data.frame(state_eviction2)
state_eviction2df
```


```{r}
purPal <- colorQuantile("PuBuGn", domain = state_eviction$eviction_filing_rate, n = 4)
...
```{r}
 ...
 leaflet() %>% 
  setView(-98.483330, 38.712046, zoom = 4) %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  addPolygons(data = state_eviction, fillColor = ~pal(.data[["renter_occupied_households"]]), 
              smoothFactor = 0.2, fillOpacity = .7, weight = 0.2,
              popup = ~popup_evic) %>%
   addLegend(pal,
             values = state_eviction[["renter_occupied_households"]],
             position = "bottomright",
             title = "Renter Occupied House")
 
 
names(state_eviction$pct_renter_occupied)
 
spatial_colnames <- as.data.frame(state_eviction)
 
spatial_colnames %>%
  dplyr::select(NAME, eviction_filing_rate, everything()) %>%
  filter (is.na(eviction_filing_rate))

table(is.na(spatial_colnames)) 

state_eviction

is.na(state_eviction$year)
...
state_eviction <- merge(spatial_state, eviction_by_state, by = "GEOID", duplicateGeoms = TRUE)
...
state_eviction_NoNAs <- sp.na.omit(state_eviction, margin = 1)

view_noNoas <- as.data.frame(state_eviction_NoNAs)
view_noNoas
```
